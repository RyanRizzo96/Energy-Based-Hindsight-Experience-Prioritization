...
class Logger(object):
    ...
    def __init__(self, folder, output_formats, comm=None):
        """
        the logger class
...
def configure(folder=None, format_strs=None):
    ...
    if folder is None:
        folder = os.path.join("new_results/initial_tests/FetchPush-v1/trial_logdir", datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
